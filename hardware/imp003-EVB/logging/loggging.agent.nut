...
function updatePlotly(temp, lux) 
{
    ...
    if (reply["message"] != "") server.log(reply["message"]);
    if (reply["warning"] != "") server.log(reply["warning"]);
    if (reply["error"] != "") server.log(reply["error"]);
...
device.on("data", function(data){updatePlotly(data.temp, data.lux);});
