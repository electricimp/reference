...
function tempToColor(temp) {
    ...
    color.grn <- 255 - ((255.0 / 19.0) * (math.abs(temp - 19.0)));
    if (color.grn < 0) { color.grn = 0; }
    if (color.grn > 255) { color.grn = 255; }

    // scale blue inversely to temp, from 0 to 20 C
    color.blu <- 255 - (255.0/20.0) * (temp);
    if (color.blu < 0) { color.blu = 0; }   
    if (color.blu > 255) { color.blu = 255; }
    
    server.log(format("Temp: %0.2f -> [%d,%d,%d]", temp, color.red, color.grn, color.blu));
