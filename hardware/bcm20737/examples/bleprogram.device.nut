...
class Eeprom24FC {
    ...
    constructor(_i2c, _wp = null, _addr = 0xA0) {
        i2c = i2c;
        addr = addr;
        wp = _wp;
    }
    ...
    function chipErase() {
        ...
        local numchunks = EEPROMSIZE / CHUNKSIZE;
        ...
        for (local i = 0; i < numchunks; i++) {
            eeprom.write(zerobuffer, i * CHUNKSIZE);
...
agent.on("zero", ble.clearExtMemory);
